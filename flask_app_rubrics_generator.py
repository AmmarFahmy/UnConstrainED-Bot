...
def ask():
    ...
    result = Rubrics_generator.llm_chain.run(
...
def send_expansion_request_to_gpt4(expansion_prompt):
    ...
    prompt_template = f"Expand the following text with real world example and incidents: \"{expansion_prompt}\""

    prompt = PromptTemplate.from_template(prompt_template)

    expansion_chain = LLMChain(
        llm=Rubrics_generator.llm, prompt=prompt)
